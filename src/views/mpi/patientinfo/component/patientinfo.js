...
export function postPatientinfo(obj) {
  return request({
    url: 'gateway/mpi/v1/patientinfo',
    method: 'post',
    data: obj
  })
}
...
export function getPatientinfo(id, method) {
  return request({
    url: 'gateway/mpi/v1/patientinfo/' + id,
    method: method || 'post'
  })
}
// 根据主键ID删除个人基本信息表一条数据
...
export function statusPatientinfo(id, status) {
  return request({
    url: 'gateway/mpi/v1/patientinfo/status/' + id + '/' + status,
    method: 'post'
  })
}
// 注销合并后的患者信息
export function cancelPatients(obj) {
  return request({
    url: 'gateway/mpi/v1/patientinfo/cancelPatients/' + obj,
    method: 'post'
  })
}
